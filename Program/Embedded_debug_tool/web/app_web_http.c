...
static esp_err_t page_handler(httpd_req_t *req)
{
    ...
    int n = snprintf(page, 5120,
        ...
        "var teon=!!parseInt(p[0]);"
        "document.getElementById('te').checked=teon;"
        "document.getElementById('ti').value=p[1];"
        "paused=parseInt(p[2]);"
        "TX=parseInt(p[3])||0;"
        "var bp=document.getElementById('bp'),sb=document.getElementById('sb');"
        "bp.textContent=paused?'继续':'暂停';bp.className=paused?'active':'';"
        "sb.disabled=teon;sb.textContent=teon?'定时中':'发送';"
        "updInfo();return}"
        "if(paused)return;"
