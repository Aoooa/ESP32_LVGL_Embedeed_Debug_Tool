...
/* launcher：桌面启动器（3 行列表选择器 + 调速拨轮），基于 LVGL 9.4。
 *
 * 结构：
 *   root（全屏背景，纯色，随主题动画渐变）
 *   ├── 选择器（3 行窗口铺满屏幕，每行 = 图标 + 名字，Miku 绿边框，无竖线；
 *   │    颜色/字号全部一致；选定框高亮橙粗边框框住选中行，跟随切换移动）
 *   ├── idx 指示（右上角 N/总数）
 *   └── wheel（右侧调速拨轮：上下拖动控制自动切换速率）
 *
 * 交互（内容原地刷新 + 选定框跟随移动）：
 *   触摸垂直滑动 → 3 行窗口内容原地更新（省略卡片滚动渲染），方向与
 *   手指一致（上滑露出下方 APP）；选定框每切换 1 次向手指反向移动 1 行
 *   （上滑 → 框 顶→中→底，到底后停住），切换次数 = 框移动次数 1:1；
 *   松手惯性按位移折算有限格数；单击：框不在点击行 → 第一击只移框，
 *   框已在 → 启动该 APP；到顶/到底边界停留。
 *
 * APP 管理：中央描述符表（app_manifests）+ 返回栈（深度 4）：
 *   启动 = 压栈（来源 APP 保活，arg 带参启动）；返回 = 栈顶 back 回调，
 *   true 弹栈回来源，false APP 内部处理。系统事件（右滑返回/旋转/SD 就绪/
 *   调试）统一路由到栈顶 APP 回调。
 *
