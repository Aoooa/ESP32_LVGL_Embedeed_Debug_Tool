...
/* launcher：桌面启动器（3 行切换选择器 + 调速拨轮），基于 LVGL 9.4。
 *
 * 结构：
 *   root（全屏背景，纯色，随主题动画渐变）
 *   ├── 选择器（中央固定 3 行：上预览/当前/下预览，每行 图标|竖线|名字，
 *   │    无卡片边框；选定框霓虹青边框框住当前行，切换时滑入吸附）
 *   ├── idx 指示（右上角 N/总数）
 *   └── wheel（右侧调速拨轮：上下拖动控制自动切换速率）
 *
 * 交互：垂直滑动切换选中（下滑=上一个、上滑=下一个，达阈值即切、可连续
 * 快速滑动，松手惯性逐格切换）；单击当前行启动 APP；到顶/到底边界停留
 * （越界预览行隐藏）。
 *
